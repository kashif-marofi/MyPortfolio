...
// Agar portfolio-details.html me ho to data load karo:
if (window.location.pathname.includes("portfolio-details.html")) {
  const details = JSON.parse(localStorage.getItem("portfolioDetails"));
  if (details) {
    // Title
    document.querySelector(".page-title h1").textContent = details.title;
    document.querySelector(".portfolio-info ul").innerHTML = `
      ...
      <li><strong>Project URL</strong>: <a href="${details.url}">${details.url}</a></li>
    `;
    ...
    swiperWrapper.innerHTML = details.imageUrls.map(url => `
      <div class="swiper-slide"><img src="${url}" alt=""></div>
    `).join("");
    
    // Initialize swiper (already auto-loaded in template)
